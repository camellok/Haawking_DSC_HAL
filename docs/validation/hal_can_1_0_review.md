...
| P2-L06 | 20416 | 184 | 284 | 20884 |
| P2-L07 | 24418 | 328 | 36 | 24782 |
...
- 当前候选已逐Lab完成P2-L01至P2-L07 IDE构建和上板功能回归；
- 标准/扩展TX、零DLC、目标帧polling RX、内部Loopback、Loopback+Silent、多对象过滤、远程帧、中断事件及Error Passive受控恢复均已有当前候选截图；
- 七个Lab结束时的`transactionTimeoutCount`和`unexpectedInterruptCount`均为0；
- 160 MHz目标时序采样已通过：消息RAM初始化101个样本的最大值为285周期；IF1/IF2分别取得1,859,832/2,303,095个样本，最大值为609/12周期；Line 0/Line 1 ISR函数体约46万个样本的最大值为924/372周期；采样期间帧错误、队列溢出、事务超时和意外中断均为0；
- IF1最大值可能包含前台测量区间被CAN ISR抢占的时间；IF1/IF2的最大BUSY轮询为0表示事务在首次BUSY循环体执行前完成。现有IF事务1000次和消息RAM初始化100000次保守上限保持不变；
- 证据身份、逐项结果和边界记录在训练仓库`Docs/lesson_notes/02_can_hal/HAL_CAN_1_0_candidate_regression.md`。

仍需完成的冻结门禁：

- [目标时序验证](hal_can_1_0_target_timing.md)使用的临时探针和重复RAM初始化激励已经移除；清理后的发布构建、目标ABI、严格告警与静态栈门禁均已重新通过；
- 推送HAL与训练仓库候选分支并完成PR审查；本轮按计划不合并`main`，正式冻结状态留待后续合并闭环确认。

## 当前未验证边界

R04错误ID拒绝与`EMPTY`不改写、standalone Silent、R10队列满、Bus-off动态进入、强制IF超时、External Loopback、长期压力和CANopenNode适配均不属于当前候选的直接证据。R10由用户明确决定不执行。它们不阻塞经风险边界声明后的初版功能基线，但不得描述为量产验证通过。
