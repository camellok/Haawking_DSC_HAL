...
- 仓库维护公共HAL接口、目标芯片实现、HAL共用的目标无关软件组件、架构规范、参考platform和验证记录。
...
- 目标无关软件组件放在`source/common`，不得包含DriverLib或芯片、外设、协议专用逻辑。
- 软件组件默认不得自行开关全局中断；若对象跨ISR和主循环共享，接口必须说明并发模型，由宿主提供临界区或其他同步。
...
- `platform.h`只暴露应用或服务真正需要的板级能力；`platform_config.h`集中外设实例、GPIO、IRQ、ACK组、时钟源、XBAR等资源连接宏，并声明用户配置实例；platform实现文件（通常为`platform.c`）在文件作用域定义这些具有静态存储期的实例。
